/*                                                  */
/*  Actividad 5.3 - Situación problema 2            */
/*                                                  */
...
// Number of threads you're willing to use
#define NUM_THREADS 10

// Reads the directory given and returns a queue with the names of all the scheme files
std::queue<std::string> searchFiles(std::string dirName)
{
    HANDLE hFind;
    WIN32_FIND_DATA FindData;

    // Filters the files and returns only the ones that have .scm extension
    dirName = dirName + "\\*.scm";

    // Copies the content of the string to a char array in order for FindFisrtFile() to work
    char buffer[dirName.length() + 1];
    strcpy(buffer,dirName.c_str());

    // Creation of the queue with .scm files
    std::queue<std::string> filesToAnalyze;

    // Find the first file of the directory
    hFind = FindFirstFile(buffer, &FindData);

    // Appends the file to the queue
    filesToAnalyze.push(FindData.cFileName);

    // Keeps appending files while there are more .scm files remaining
    ...
    // Returns the queue with all the .scm files name
...
// Analyzes the syntax of an .scm file and outputs a html file with it's corresponding colors
void lexerArithmetic(std::string filePath, std::string fileName)
{
    std::ifstream expressionsFile;                                                                              // Input file given by user
    std::ofstream outputFile;                                                                                   // Output file with the html code
    std::string outputFileName;                                                                                 // Name that the current output file will have
    ...
    // Creates the correct file path
    filePath = filePath + "\\" + fileName;
    // Opens the file to analyze given by user
    expressionsFile.open(filePath, std::ios::in);

    // Modifies the file name for it to become html
    fileName.erase(fileName.end()-4, fileName.end());
    fileName = fileName + ".html";
    outputFileName = fileName;
    // Open/Creates the output file
    outputFile.open(outputFileName, std::ios::out);
    // Prints all the html info and required tags to the output file
    ...
    // Prints all the html info and required closing tags to the output file
...
int main(void)
{
    std::cout << "\n |-|-|      DFA SYNTATIC HIGHLIGHTER WITH THREADS      |-|-|" << std::endl;
    std::cout << "By: Aldo Degollado, Abraham Mendoza, and Alonso Martinez\n" << std::endl;

    // Directory of files that you're willing to analyze
    std::string pathToAnalyze = "C:\\Users\\aldod\\OneDrive\\Escritorio\\test\\Pruebas";

    // Begins the time measurement
    auto t1 = std::chrono::high_resolution_clock::now();

    // Producer thread creation and execution
    std::queue<std::string> filesInQueue;
    std::thread producerThread([&] {filesInQueue = searchFiles(pathToAnalyze);});
    producerThread.join();

    // Consumer threads creation and execution
    std::thread consumerThreads[NUM_THREADS];
    int counter = 0;

    // The consuming threads parse every file found by the producer thread
    while(!filesInQueue.empty())
    {
        // When the first NUM_THREADS of threads finish their analysis, another batch of the same amount of threads is created
        if(counter >= NUM_THREADS)
        {
            std::thread consumerThreads[NUM_THREADS];
            counter = 0;
        }
        // A file is taken and removed of the queue
        std::string fileToAnalyze = filesInQueue.front();
        filesInQueue.pop();
        // The file took of the queue is assigned to a consuming thread
        consumerThreads[counter] = std::thread(lexerArithmetic, pathToAnalyze, fileToAnalyze);
        counter = counter + 1;
    }

    // When the queue gets empty, we wait for all the consuming threads that are still working to finish their analysis
    ...
    // Converts the time measured to milliseconds
    auto duration = std::chrono::duration_cast<std::chrono::seconds>(t2 - t1).count();
    std::chrono::duration<double, std::milli> ms_double = t2 - t1;
    std::cout << "It took " << ms_double.count() << " milliseconds." << std::endl;
